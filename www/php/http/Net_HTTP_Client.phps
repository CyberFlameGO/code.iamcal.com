...
/*
	...
	hacked in places by cal henderson
	now supports timing out, which php sucks at
...
class http_client 
{
	 ...
	 * @seeAlso connect
	 **/	
	function http_client( $host= NULL, $port= NULL )
	{
...
//			$this->addHeader( "cookie", $this->responseHeaders['set-cookie'] );
	...
	function processHeader( $lastLine = CRLF )
	{
		...
		while ( ( ! $finished ) && ( ! feof($this->socket)) && ($header_count < 20)) {
			$header_count++;
